...
企业内部调研与考核平台，支持多类型问卷创建、白名单管控、企微 OAuth 授权填写、免登录外部问卷、数据统计与导出。
...
- **免登录填写（外部问卷）**：问卷考核 / 案例收集可开启「免登录填写」开关，开启后发布的问卷任何人凭链接直接填写、无需企业微信登录（匿名、不限次），适用于对外征集与调研；此时不适用白名单，数据以「提交记录 + 计数」呈现，提交人显示为「外部填写 #序号」。开关默认关闭（走企微登录）
...
Survey           问卷（含 public_fill 免登录填写开关）
...
- 「免登录填写」开关仅对问卷考核 / 案例收集开放；开启后填写端走公开接口 `POST /api/survey/:shareToken/public-submit`（无鉴权，后端强制校验 `public_fill=true` 才放行），匿名提交以 `ext-<随机>` 作为提交标识，不查白名单、不做每人限填
- 本功能新增数据库迁移 `20260714000000_add_survey_public_fill`（给 `surveys` 加 `public_fill` 列），部署时需执行 `npx prisma migrate deploy`
